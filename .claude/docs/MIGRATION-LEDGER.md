...
| Generic object REST READ (metadata-driven) | 🔁 | | Slice 8: `app/lib/records.ts` + `app/rest/[object]/route.ts` (list) + `[object]/[id]` (get-one) serve **any** standard object's SCALAR fields, columns derived from `fieldMetadata` (resolve by `namePlural` → table → active non-system scalar columns; `deletedAt IS NULL`; `id ASC`; identifier-guarded). Envelopes keyed dynamically (`data.{namePlural}`/`data.{nameSingular}`); auth + read-permission gated; non-GET verbs proxy to legacy (no 405). Live-verified vs the `workflow` object (2 rows) in-process + HTTP; companies static routes unaffected. **Deferred:** generic writes, composite/relation/enum fields, filter/order/depth/pagination, folding companies in |
