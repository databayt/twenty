...
| Generic object REST CRUD (metadata-driven) | 🔁 | | Slices 8–9: `app/lib/records.ts` + `app/rest/[object]/route.ts` (GET list, POST create) + `[object]/[id]` (GET/PATCH+PUT/DELETE) serve full CRUD for **any** standard object's SCALAR fields, all derived from `fieldMetadata` (resolve by `namePlural` → table; writable = active non-system scalars minus POSITION; per-type value casts text/uuid/double/numeric/boolean/timestamptz/date/jsonb; unknown-field/empty/bad-type → 400; required-field pre-check; createdBy* omitted → DB default; DELETE hard-by-default / `?soft_delete=true` soft; identifier-guarded). Envelopes keyed dynamically (`data.{namePlural}`/`data.create{Singular}`/`update{Singular}`/`delete{Singular}`); auth + per-op permission (read/create/update/softDelete/destroy); system objects + reserved segments proxy to legacy. Live-verified vs `workflow` (read) + `note` (write) in-process + HTTP, net-zero; companies static routes unaffected. **Slice 11** = composite-field **READ** expansion in the generic reader (the two GET routes only): records.ts +`COMPOSITE_TYPE_DEFINITIONS` (ACTOR/ADDRESS/CURRENCY/EMAILS/FULL_NAME/LINKS/PHONES/RICH_TEXT — sub-column = `field.name+Capitalize(prop)`) +`getTableColumnSet` (drift guard) +`getObjectReadShape` (scalars byte-identical to Slice 8 + composite SELECT plan; system composites skipped except allowlisted audit ACTORs `createdBy`/`updatedBy`) +`reNestRecord` (collapses flat sub-columns back to nested objects, twenty's TwentyORM formatResult null-equivalents: TEXT→`''`, jsonb-array→`[]`, jsonb-obj→`{}`, numeric→raw/coerced; ACTOR collapses to default/null when required props empty). `listRecords`/`getRecord` gained an optional `composites=[]` (empty ⇒ byte-identical to before). Live-verified vs `company` (live LINKS/ADDRESS/CURRENCY/ACTOR data) + a net-zero `person` probe (Harry Potter, nested name/emails/phones/linkedinLink/createdBy) in-process + HTTP. **Deferred:** composite/relation/enum **writes** (POST/PATCH still scalar-only, echo flat), relation/MORPH read expansion, SELECT/MULTI_SELECT/RATING, FILES, batch/restore/groupBy/merge, filter/order/depth/pagination, folding companies in |
